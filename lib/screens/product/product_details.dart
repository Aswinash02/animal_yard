...
class _ProductDetailsState extends State<ProductDetails>
  ...
  bool _buyNowLoadingState = false;
  ...
  addToWishList() async {
    var wishListCheckResponse = await WishListRepository()
        .add(productId: _productDetails!.id.toString());
    print('wishListCheckResponse ${wishListCheckResponse.is_in_wishlist}');
    //print("p&u:" + widget.slug.toString() + " | " + _user_id.toString());
    _isInWishList = wishListCheckResponse.is_in_wishlist;
    setState(() {});
  }
  ...
  onPressBuyNow(context) async {
    _buyNowLoadingState = true;
    setState(() {});
    await addToCart(mode: "buy_now", context: context);
    _buyNowLoadingState = false;
    setState(() {});
  }
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Btn.minWidthFixHeight(
                          ...
                          onPressed: () {
                            if (sellerChatMessageController.text == "") {
                              ToastComponent.showDialog(
                                  AppLocalizations.of(context)!
                                      .title_or_message_empty_warning,
                                  gravity: Toast.center,
                                  duration: Toast.lengthLong);
                            } else {
                              Navigator.of(context, rootNavigator: true).pop();
  ...
  Future<void> onTapSellerCall() async {
    try {
      final Uri url = Uri(scheme: 'tel', path: _productDetails?.phone);
  ...
  Widget build(BuildContext context) {
    print(' _productDetails?.added_by   ${_productDetails?.added_by}');
      ...
      child: Row(
        children: [
          ...
          Padding(
            ...
            child: Text(
              convertPrice(_totalPrice.toString()),
              style: TextStyle(
                  color: MyTheme.accent_color,
                  fontSize: 16.0,
                  fontWeight: FontWeight.w600),
  ...
  Widget buildAdminBottomAppBar(BuildContext context, _addedToCartSnackbar) {
  ...
  Widget buildSellerBottomAppBar() {
    return BottomNavigationBar(
      backgroundColor: MyTheme.white.withOpacity(0.9),
      items: [
        BottomNavigationBarItem(
  ...
  Future openPhotoDialog(BuildContext context, String imageUrl) {
    return showDialog(
      context: context,
  ...
  Widget buildProductSliderImageSection(BuildContext context) {
    if (_productImageList.isEmpty) {
      ...
      return CarouselSlider(
        ...
        items: _productImageList.map((image) {
          return Container(
            child: Stack(
              children: <Widget>[
                InkWell(
