...
class _OfflineState extends State<OfflineScreen> {
  ...
  onPressSubmit() async {
    ...
    if (name == "") {
      ToastComponent.showDialog(AppLocalizations.of(context)!.name,
          gravity: Toast.center, duration: Toast.lengthLong);
      return;
    }
    if (trx_id == "") {
      ToastComponent.showDialog(AppLocalizations.of(context)!.transaction_id,
          gravity: Toast.center, duration: Toast.lengthLong);
      return;
    }
    ...
    if (widget.offLinePaymentFor == PaymentFor.WalletRecharge) {
      ...
      var submitResponse =
        ...
        trx_id: trx_id,
        photo: _photo_upload_id,
        addressId: widget.shippingInfoId,
      ...
      if (submitResponse.result == false) {
        ...
        Navigator.push(context, MaterialPageRoute(builder: (context) {
          return OrderDetails(
              id: submitResponse.combined_order_id, go_back: false);
        }));
      }
